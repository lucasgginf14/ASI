...
import es.udc.asi.bnbria_rest.property.service.dto.PropertyStatsRequest;
import es.udc.asi.bnbria_rest.property.service.dto.PropertyStatsView;
...
public class PropertyService {
  ...
  @Transactional(readOnly = true)
  public List<PropertyStatsView> findPropertyStats(PropertyStatsRequest request) {
    return propertyDao.findPropertyStatsByCitiesAndYear(request.cities(), request.year())
      .stream()
      .map(row -> new PropertyStatsView(
        (String) row[0],
        (String) row[1],
        (Long) row[2],
        (Double) row[3]
